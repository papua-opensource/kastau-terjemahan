...
async function translate(): Promise<void> {
  ...
  try {
        ...
        <div class="flex flex-col bg-white border shadow-sm rounded-xl">
          ...
          ${results[0].contoh_asal && results[0].contoh_terjemahan ? `
              ...
              <div>
                  ...
                  >
                      ${sourceLang === 1 ? `Kalimat Sentani` : `Kalimat Mooi`}
                  </h3>
                  <p
    ...
    function speakTextWithGoogleCloud() {
      const text = targetWord.querySelector("textarea")
      ...
      fetch(
        `https://texttospeech.googleapis.com/v1/text:synthesize?key=${import.meta.env.PUBLIC_GOOGLE_CLOUD_API_KEY
        }`,
        {
          method: "POST",
          headers: {
