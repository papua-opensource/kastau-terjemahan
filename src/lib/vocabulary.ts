...
async function displayDataForLetter(letter: string, lang_id: number) {
    ...
    if (data && data.length > 0) {
        data.forEach((item, index) => {
            ...
            const markedAsal = highlightWords(item.contoh_asal, item.kata);
            // Menandai kata di contoh_terjemahan
            const markedTerjemahan = highlightWords(item.contoh_terjemahan, item.arti);
            modalDiv.innerHTML = `
                <div class="hs-overlay-open:mt-7 hs-overlay-open:opacity-100 hs-overlay-open:duration-500 mt-0 opacity-0 ease-out transition-all sm:max-w-lg sm:w-full m-3 sm:mx-auto">
                    <div class="flex flex-col bg-white border shadow-sm rounded-xl">
                        ...
                        ${item.contoh_asal && item.contoh_terjemahan ? `
                            <div>
                                <h3
                                    class="text-gray-900 font-inter font-semibold"
                                >
